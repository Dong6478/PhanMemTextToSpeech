...
running_threads = []  # Danh sách theo dõi các luồng đang chạy
...
def request_new_api_key():
    ...
    if new_key:
        ...
        new_key = giai_ma(chuoi_can_giai_ma= new_key, optional= 0)
        ma_hoa(new_key)
        messagebox.showinfo("Thông Báo", "Thay key mới thành công!")
        return new_key
    return None
    ...
    try:
        ...
        with open(audio_file_path, "rb") as audio_file:
            ...
            try:
                ...
                messagebox.showerror("HẾT HẠN KEY ĐĂNG KÝ", f"Lỗi API: {e}")
                new_key = request_new_api_key()
                return None
    except:
        messagebox.showerror("THÔNG BÁO","Chưa nhập key vui lòng nhập key để sử dụng")
        new_key = request_new_api_key()
        return None
# Hàm xóa tệp tạo ra
...
def close_app():
    # Đặt logic để dừng tất cả các tiến trình và luồng đang chạy
     # Dừng tất cả các luồng đang chạy
    for thread in running_threads:
        if thread.is_alive():
            # Đặt logic để dừng luồng ở đây
            pass
